...
let RedisTokenRepository = class RedisTokenRepository {
    ...
    buildKey(turfId, date, startTime, endTime) {
        return `lock:slot:${turfId}:${date}:${startTime}-${endTime}`;
    }
    aquireLock(turfId, date, startTime, endTime, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const key = this.buildKey(turfId, date, startTime, endTime);
            const result = yield redis_client_1.redisClient.set(key, userId, {
                NX: true,
                EX: 600
            });
            return result === "OK";
    ...
    verifyLock(turfId, date, startTime, endTime, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const key = this.buildKey(turfId, date, startTime, endTime);
            const owner = yield redis_client_1.redisClient.get(key);
            return owner === userId;
        });
    }
    releaseLock(turfId, date, startTime, endTime, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const key = this.buildKey(turfId, date, startTime, endTime);
            const owner = yield redis_client_1.redisClient.get(key);
            if (owner === userId) {
