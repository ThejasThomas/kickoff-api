...
implements ICancelRequestRepository {
    ...
    async getCancelRequestByOwnerId(ownerId: string): Promise<ICancellationRequestEntity[]> {
        const request= await  CancellationRequestModel.find({ownerId}).sort({createdAt:-1})
        const result=[]
